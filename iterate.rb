...
def days_in_month(input)
  mth = [nil, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
  y = Date.parse(input).strftime('%Y')
  m = Date.parse(input).strftime('%m')
  m[2] = 29 if m == '02' && (y.to_i % 4).zero?
  mth[m.to_i]
end
